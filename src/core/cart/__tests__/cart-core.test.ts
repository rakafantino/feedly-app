...
describe("calculateAveragePrice", () => {
  it("calculates average based on total quantity, not item count", () => {
    const items: CartItemType[] = [
      { id: "1", productId: "p1", name: "A", price: 10000, quantity: 2, unit: "pcs" },
      { id: "2", productId: "p2", name: "B", price: 20000, quantity: 2, unit: "pcs" },
    ];
    // Item A: 10000 * 2 = 20000
    // Item B: 20000 * 2 = 40000
    // Subtotal = 60000, Total quantity = 4
    // Average = 60000 / 4 = 15000 (per unit quantity)
    expect(calculateAveragePrice(items)).toBe(15000);
  });
...
describe("validateCartForCheckout", () => {
  ...
  it("returns error for invalid quantity", () => {
    const items: CartItemType[] = [{ id: "1", productId: "p1", name: "A", price: 10000, quantity: 0, unit: "pcs" }];
    const result = validateCartForCheckout(items);
    expect(result.valid).toBe(false);
  });
});
...
describe("prepareCartExport", () => {
  it("prepares export data", () => {
    const items: CartItemType[] = [{ id: "1", productId: "p1", name: "Product A", price: 10000, quantity: 2, unit: "pcs" }];
