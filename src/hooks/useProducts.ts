...
export function useProducts(params?: UseProductsParams) {
  const { page = 1, search, category, lowStock, excludeRetail, retailOnly, limit = 10, minimal, enabled = true, storeId: paramStoreId } = params || {};
  ...
  return useQuery({
    queryKey: ["products", page, search, category, lowStock, excludeRetail, retailOnly, limit, minimal, effectiveStoreId],
    queryFn: async (): Promise<ProductsResponse> => {
      ...
      if (excludeRetail) searchParams.set("excludeRetail", "true");
      if (retailOnly) searchParams.set("retailOnly", "true");
...
export function useConvertInventory() {
  ...
  return useMutation({
    mutationFn: async (payload: { productId: string; quantity: number; unit: string; convertedUnit?: string; convertFromBatch?: boolean }) => {
...
export function useSetupRetail() {
  ...
  return useMutation({
    mutationFn: async (payload: { productId: string; convertFromBatch: boolean; retailProductName: string; retailProductPrice: number; quantityPerUnit: number; convertImmediately: boolean }) => {
