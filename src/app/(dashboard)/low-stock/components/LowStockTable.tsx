...
import { ArrowUpDown, Filter, Loader2, Plus, Search, FilterX, ShoppingCart, Check } from "lucide-react";
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          <div>
            ...
            <CardDescription>{products.length} produk di bawah threshold stok</CardDescription>
          </div>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="default" size="sm" onClick={createPurchaseOrder} disabled={Object.values(selectedProducts).filter(Boolean).length === 0}>
                  ...
                  <TableRow>
                    ...
                    <TableHead className="cursor-pointer" onClick={() => toggleSort("name")}>
                      <div className="flex items-center space-x-1">
                        <span>Produk</span>
                        {sortColumn === "name" && <ArrowUpDown className={`h-4 w-4 ${sortDirection === "desc" ? "rotate-180" : ""} transition-transform`} />}
                      </div>
                    </TableHead>
                    <TableHead className="cursor-pointer" onClick={() => toggleSort("category")}>
                      <div className="flex items-center space-x-1">
                        <span>Kategori</span>
                        {sortColumn === "category" && <ArrowUpDown className={`h-4 w-4 ${sortDirection === "desc" ? "rotate-180" : ""} transition-transform`} />}
                      </div>
                    </TableHead>
                    <TableHead className="cursor-pointer" onClick={() => toggleSort("stock")}>
                      <div className="flex items-center space-x-1">
                        <span>Stok</span>
                        {sortColumn === "stock" && <ArrowUpDown className={`h-4 w-4 ${sortDirection === "desc" ? "rotate-180" : ""} transition-transform`} />}
                      </div>
                    </TableHead>
                    ...
                    <TableHead className="cursor-pointer text-right" onClick={() => toggleSort("price")}>
                      <div className="flex items-center justify-end space-x-1">
                        <span>Harga</span>
                        {sortColumn === "price" && <ArrowUpDown className={`h-4 w-4 ${sortDirection === "desc" ? "rotate-180" : ""} transition-transform`} />}
                      </div>
                    </TableHead>
                  ...
                  {filteredProducts.length > 0 ? (
                    filteredProducts.map((product) => (
                      <TableRow key={product.id}>
                        ...
                        <TableCell>
                          <Button variant={selectedProducts[product.id] ? "default" : "outline"} size="icon" className="h-7 w-7" onClick={() => toggleProductSelection(product.id)}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            >
                              {incrementLoading[product.id] ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4 mr-1" />}
                              Tambah
                            </Button>
                          </div>
                        </TableCell>
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className="h-24 text-center">
                        {searchTerm || categoryFilter !== "all" || statusFilter !== "all" ? (
                          ...
                          <div className="text-sm text-muted-foreground">Semua produk memiliki stok yang cukup</div>
                        )}
                      </TableCell>
                    </TableRow>
                  )}
            ...
            <div className="grid grid-cols-1 gap-3 sm:hidden">
                ...
                filteredProducts.map((product) => (
                  <div key={product.id} className="border rounded-lg p-3">
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex-1">
                        <h3 className="font-medium line-clamp-1">
                          <div className="flex items-center">
                            <Button variant={selectedProducts[product.id] ? "default" : "outline"} size="icon" className="h-6 w-6 mr-2" onClick={() => toggleProductSelection(product.id)}>
                      ...
                      <div className="ml-2">{renderStockStatus(product)}</div>
                    </div>
                    ...
                    <div className="flex items-center mt-2">
                      ...
                      <Button
                        ...
                        disabled={incrementLoading[product.id]}
                      >
                        {incrementLoading[product.id] ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <Plus className="h-4 w-4 mr-1" />}
                        Tambah Stok
                      </Button>
                    </div>
                  </div>
              ...
              {filteredProducts.length > 0 && Object.values(selectedProducts).some(Boolean) && (
                <div className="sticky bottom-4 flex justify-center z-10">
                  <Button variant="default" size="default" onClick={createPurchaseOrder} className="shadow-md">
                    <ShoppingCart className="h-4 w-4 mr-2" />
