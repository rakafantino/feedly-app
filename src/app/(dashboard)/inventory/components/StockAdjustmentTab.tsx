...
export default function StockAdjustmentTab({ products, onRefresh }: StockAdjustmentTabProps) {
  ...
  // Computed: check if there are batches with stock > 0
  const hasActiveBatches = productBatches.some((b) => b.stock > 0);
  ...
  useEffect(() => {
    if (!searchQuery.trim()) {
      ...
      setFilteredProducts(products.filter((p) => p.name.toLowerCase().includes(query) || (p.category?.toLowerCase() || "").includes(query)));
    }
  ...
  const handleSubmit = async () => {
    ...
    // Validate batch selection for products with batches that have stock > 0
    if (hasActiveBatches && !selectedBatchId) {
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="relative mb-4">
          ...
          <Input placeholder="Cari produk..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9" />
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4 flex items-start gap-2">
          ...
          <div className="text-sm text-blue-800">
            ...
            <p className="text-blue-700 mt-1">Gunakan fitur ini untuk koreksi stok setelah stock opname, mencatat barang rusak, atau menghapus barang kadaluarsa. Semua penyesuaian akan tercatat di Laporan Penyesuaian Stok.</p>
          </div>
        </div>
              ...
              {filteredProducts.length === 0 ? (
                ...
                filteredProducts.slice(0, displayCount).map((product) => (
                  <TableRow key={product.id}>
                    ...
                    <TableCell className="text-right">{product.purchase_price ? formatRupiah(product.purchase_price) : "-"}</TableCell>
                    <TableCell>
        ...
        {filteredProducts.length > displayCount && (
          <div className="mt-3 text-center">
            <Button variant="outline" size="sm" onClick={() => setDisplayCount((prev) => prev + 10)}>
        ...
        <Dialog open={openDialog} onOpenChange={setOpenDialog}>
          <DialogContent className="max-w-md max-h-[90vh] flex flex-col">
            <DialogHeader className="shrink-0">
              ...
              <DialogDescription>Catat perubahan stok untuk {selectedProduct?.name}</DialogDescription>
            </DialogHeader>
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="grid gap-4 py-4">
                ...
                ) : productBatches.filter((b) => b.stock > 0).length > 0 ? (
                  <div className="space-y-2">
                    <Label>
                    ...
                    <p className="text-xs text-muted-foreground">Produk ini menggunakan sistem batch. Pilih batch yang akan di-adjust.</p>
                  </div>
                ) : (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                    <p className="text-sm text-amber-800">Semua batch untuk produk ini sudah habis stok. Penyesuaian pengurangan tidak memerlukan pemilihan batch.</p>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Button type="button" variant={!isPositive ? "default" : "outline"} className="flex-1" onClick={() => setIsPositive(false)}>
                      − Kurangi
                    </Button>
                    <Button
                      type="button"
                ...
                <div className="space-y-2">
                  ...
                  <Textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Contoh: Selisih hasil stock opname, barang pecah saat handling..." rows={2} />
                </div>
              </div>
            </div>
            ...
            <DialogFooter className="shrink-0 pt-4 border-t">
              ...
              <Button onClick={handleSubmit} disabled={isSubmitting || !quantity || (hasActiveBatches && !selectedBatchId)}>
