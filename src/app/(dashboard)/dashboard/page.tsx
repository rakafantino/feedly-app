...
export default function DashboardPage() {
  ...
  const getPeakTransactionTime = () => {
    ...
    const peakHour = dashboardData.hourlyTransactions.reduce((max: HourlyTransaction, current: HourlyTransaction) => (current.transactions > max.transactions ? current : max), dashboardData.hourlyTransactions[0]);
    ...
    <div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <p className="text-xs text-muted-foreground">
              {timeFilter === "day" && (
                <>
                  ...
                  {dashboardData.percentageChange.toFixed(1)}% dari kemarin
                </>
              )}
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{timeFilter === "day" ? dashboardData.averageMargin?.toFixed(1) || "0" : dashboardData.currentPeriodMargin?.toFixed(1) || "0"}%</div>
            <p className="text-xs text-muted-foreground">
              {timeFilter === "day" && (
                <>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Produk perlu diisi ulang</p>
          </CardContent>
        </Card>
      </div>
        ...
        <TabsContent value="overview" className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:justify-between mb-4 gap-3">
            <div className="flex justify-center sm:justify-start w-full sm:w-auto">
              <div className="inline-flex rounded-md border p-1 shadow-sm w-full sm:w-auto">
                <button onClick={() => setTimeFilter("day")} className={`flex-1 px-3 py-1.5 text-sm ${timeFilter === "day" ? "bg-primary text-primary-foreground" : "text-muted-foreground"} rounded-sm transition-colors`}>
                  Hari
                </button>
                <button onClick={() => setTimeFilter("week")} className={`flex-1 px-3 py-1.5 text-sm ${timeFilter === "week" ? "bg-primary text-primary-foreground" : "text-muted-foreground"} rounded-sm transition-colors`}>
                  Minggu
                </button>
                <button onClick={() => setTimeFilter("month")} className={`flex-1 px-3 py-1.5 text-sm ${timeFilter === "month" ? "bg-primary text-primary-foreground" : "text-muted-foreground"} rounded-sm transition-colors`}>
                  Bulan
                </button>
              </div>
            </div>
          ...
          <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-7">
            ...
            <Card className="lg:col-span-2">
              ...
              <CardContent className="space-y-2">
                ...
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-primary" style={{ width: `${Math.min(100, ((dashboardData.currentPeriodTotal || 0) / (dashboardData.salesTarget || 1)) * 100)}%` }}></div>
                </div>
            ...
            <Card className="lg:col-span-3">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Produk Hampir Kadaluwarsa</CardTitle>
                ...
                {(dashboardData.expiringProducts?.length || 0) > 0 ? (
                  <div className="space-y-3">
                    ...
                    {(dashboardData.expiringProducts?.length || 0) > 3 && (
                      <Button variant="link" size="sm" className="w-full mt-2" onClick={() => router.push("/inventory")}>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card className="col-span-2">
              <CardHeader>
                ...
                <CardDescription>
                  {timeFilter === "day" ? "Total penjualan per jam dalam 24 jam terakhir" : timeFilter === "week" ? "Total penjualan per hari dalam seminggu terakhir" : "Total penjualan per minggu dalam sebulan terakhir"}
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  {loading ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        <YAxis tick={{ fontSize: 12 }} tickFormatter={(value) => (value >= 1000000 ? `${(value / 1000000).toFixed(0)} Jt` : `${(value / 1000).toFixed(0)} Rb`)} />
          ...
          <div className="grid gap-4 grid-cols-1">
            ...
            <Card>
              ...
              <CardContent>
                <div className="h-[250px]">
                  {loading ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        <CartesianGrid strokeDasharray="3 3" vertical={false} />
                        <XAxis dataKey="hour" tick={{ fontSize: 12 }} interval={0} angle={-45} textAnchor="end" />
                        ...
                        />
                        <Bar dataKey="transactions" fill="#8884d8" radius={[4, 4, 0, 0]} maxBarSize={40} />
                      </RechartsBarChart>
                    </ResponsiveContainer>
                  ) : (
                    <div className="h-full flex items-center justify-center">
                ...
                <p className="text-xs text-muted-foreground pt-4 text-center">{getPeakTransactionTime()}</p>
              </CardContent>
            </Card>
                              ...
                              {dashboardData.topProducts.byQuantity.map((product) => (
                                <tr key={product.id} className="border-b">
                                  <td className="px-2 py-2">
                                    <div>
                                      <div className="font-medium text-sm truncate max-w-[110px] sm:max-w-none" title={product.name}>
                                        {product.name}
                                      </div>
