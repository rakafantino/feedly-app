  ...
  async (req: NextRequest, session: any, storeId: string | null) => {
    try {
      ...
      if (!parentProductId || !conversionRate || !retailUnit) {
        return NextResponse.json({ error: "Missing required fields: parentProductId, conversionRate, retailUnit" }, { status: 400 });
      }
      ...
      const result = await prisma.$transaction(async (tx: any) => {
        ...
        const newRetailProduct = await tx.product.create({
          data: {
            ...
            hpp_price: parentProduct.purchase_price ? Math.round(parentProduct.purchase_price / parseFloat(conversionRate)) : undefined, // Explicitly set Clean HPP (No Margin)
