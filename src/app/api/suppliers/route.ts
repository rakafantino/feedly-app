...
export const GET = withAuth(
  async (request: NextRequest, session, storeId) => {
    try {
      if (!storeId) {
        return NextResponse.json({ error: "Store ID diperlukan" }, { status: 403 });
      }
...
export const POST = withAuth(
  async (request: NextRequest, session, storeId) => {
    try {
      ...
      if (!storeId) {
        return NextResponse.json({ error: "Store ID diperlukan untuk membuat supplier" }, { status: 400 });
      }
      ...
      if (error.code === "P2002" && error.meta?.target?.includes("code")) {
        return NextResponse.json({ error: "Kode Supplier sudah digunakan oleh supplier lain" }, { status: 400 });
      }

      return NextResponse.json({ error: "Terjadi kesalahan saat membuat supplier baru" }, { status: 500 });
    }
  },
  { requireStore: true },
);
