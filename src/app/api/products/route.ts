...
export const POST = withAuth(
  async (request: NextRequest, session, storeId) => {
    try {
      ...
      const isBusinessError = error.message === "Supplier tidak ditemukan atau tidak termasuk dalam toko Anda" || error.message === "Barcode sudah digunakan oleh produk lain di toko Anda";

      return NextResponse.json({ error: error.message || "Terjadi kesalahan saat menambahkan produk" }, { status: isBusinessError ? 400 : 500 });
    }
  },
  { requireStore: true },
);
