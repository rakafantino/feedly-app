    ...
    try {
      ...
      const isBusinessError =
        ...
        (error.message.includes("stock") ||
          error.message.includes("Stock mismatch") ||
          error.message.includes("batches") ||
          error.message.includes("sync") ||
          error.message.includes("repair") ||
          error.message.includes("total") ||
          error.message.includes("found") ||
          error.message.includes("Customer"));

      if (error instanceof Error) {
        return NextResponse.json({ error: error.message }, { status: isBusinessError ? 400 : 500 });
      }
